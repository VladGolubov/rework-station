...
const int miso         = 12;                           // Определяем константу с указанием № вывода Arduino к которому подключён вывод DO  ( SO, MISO ) модуля на чипе MAX6675
const int sck          = 13;                          // Определяем константу с указанием № вывода Arduino к которому подключён вывод CS  ( SS )       модуля на чипе MAX6675
const byte max6675_num = 3;
const int max6675_cs_pins[max6675_num] = {1, 4, 5};
...
void setup() {
  ...
  pid_fen.tune(1, 1, 1);
  pid_fen.limit(0, 255);
  pid_solder.tune(75, 0.07, 0.2); // Tune the PID, arguments: kP, kI, kD
